...
- **Couple sync 配慮 (Phase 3 以降)**: この 3 件は owner / partner の 2 人が同じ画面で
  side-by-side で評価する前提のため、「**3 件全部に 2 人とも同じ感想を持つ**」よりも
  「**好みが分かれる軸** (例: 雰囲気重視 vs アクセス重視 vs 料理重視) を 1 つずつ代表させる」
  方が会話が生まれる。具体的には 3 件が以下のようにレンジを持つことを目安にする:
    - 1 件は「style 完全合致 / 予算中位」 (= 主軸候補)
    - 1 件は「予算上限ギリギリだが格上 (装飾 / アクセス) で会話を膨らませる枠」
    - 1 件は「予算下位 + 異なる雰囲気で比較対照を出す枠」
  この 3 軸散らしは Diversity の上位互換 — 同タイプ回避だけでなく "話す材料の散らし方" を
  意識する。

...
- 賛否が分かれる軸はどちらかに倒さない。「料理は『品数が多い』とする声がある一方、『テンプレ的でこの式場ならでは感が薄い』とする声もある」のように両論を残す。`,
  ...
  // 30K char cap (was 50K). Mirrors SONNET_SINGLE_PAGE_INPUT_CAP and
  // the existing 25K cap in extractAcrossPages — Sonnet 60s timeout
  // was failing on 50K dense pages, surfacing as "AI 分析を取得でき
  // ませんでした". Caller-side slice acts as a defensive upper bound;
  // this slice is the floor enforced for any caller that forgets to
  // pre-truncate.

...
// Re-declared inline (no shared module) to keep the email layer self-contained
// and free of runtime React / CSS-var dependencies.
const BRAND = {
  bg: "#FAF6EE",
  card: "#FFFFFF",
  fg: "#3B3229",
...
> = {
  ...
  before_departure: {
    subject: (venue) => `そろそろ出発の時間です — ${venue}（Haretoki）`,
    lead: "見学開始まで、あと少し。\n忘れ物がないか、もう一度だけ確認していきましょう。",
    cta: "見学ページを開く",
  },
};
...
function formatJstDateTime(d: Date): string {
  // Same JST offset trick used in visit-reminders.ts — kept inline here to
  // avoid coupling email-side rendering to the cron-side classifier.

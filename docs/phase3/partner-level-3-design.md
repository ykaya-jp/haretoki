...
**状態**: 🟢 **wave 1 (Realtime broadcast scaffold) 実装完了** — wave 2 (Push) と
wave 3 (conflict resolution) は観測データ待ち
...
- [ ] `realtime.messages` の RLS policy migration (wave 2 同梱で OK)
- [ ] `/admin/onboarding-funnel` に L3 セクション (`rating_saved` / `note_added` /
  `decision_made` / `wedding_date_updated` の broadcast 件数) を追加
- [ ] 2 ブラウザでの実機 verify (dev で 1 回、prod deploy 後に 1 回)
- [ ] § 8 観測トリガーの数値収集 (1 ヶ月)

...
- AI prompts inline → モジュール化 (Phase 2 D 残課題): `URL_EXTRACTION_SYSTEM_PROMPT` / `ESTIMATE_EXTRACT_SYSTEM_PROMPT` を `src/lib/prompts/*.ts` に切り出す
- AI prompts md 化 (残): matrix-insight / fit-reason / ritual / vibe-suggest
...
- harness-ai-maintenance-plan Phase 3: docs-curator subagent
- ADR 導入 (`docs/harness/adr/`)
- MCP 運用 doc (`.claude/.mcp.json` 本実装)
- View Transitions API (Tier 3 X-9)

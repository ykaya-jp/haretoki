...
| ~~W18-6~~ | ~~**V-9: 星評価 0.5 刻み水平バー + 楽観更新**~~ → ✅ 完了 (2026-04-30 + 2026-05-06): 0.5 刻み水平バー + setRatings 楽観更新は既に実装済 (`rating-section.tsx` の `RatingBar` + `HALF_STEPS` + `lastSavedRef` 失敗時 dim-scoped rollback + `formErrors` 詳細エラートースト, commit `b7d19f4`)。**2026-05-06**: useOptimistic 試作 branch `feat/w18-rating-optimistic` (c673307) は現実装と機能的に同等 (lastSavedRef pattern で楽観更新 + rollback 達成済) と判断、stale branch 削除。useOptimistic 移行は React 19 idiom 整理時に再検討 (低優先) | audit-master-A V-9 | ✅ 完了 |
...
- ✅ **Partner Level 2 (6 次元星評価)** (確認 2026-05-05): `partner-comparison-summary.tsx` + `dimension-ratings.tsx` + `partner-can-rate-hint.tsx` 配線済 (wave 1.3 polish era)
- ✅ **Partner Level 3 (フルアプリ・全機能、Realtime / Push 込み)** (確認 2026-05-05): wave 1+ 4 で実装済、`docs/archive/2026-05/partner-level-3-design.md` に集約
- ✅ **Supabase Realtime** (確認 2026-05-05): `src/lib/push/dispatch-realtime.ts` で realtime broadcast → push fan-out 配線済 (Phase 3 L3 wave 2)
- ✅ **パートナー Push 通知** (確認 2026-05-05): `src/lib/push/{send,subscription,dispatch-realtime}.ts` + `src/server/actions/push-subscription.ts` (saveSubscription / removeSubscription / listMySubscriptions) + `src/components/notifications/push-permission-state.tsx` 全配線済、`web-push@3.6.7` 使用、PushSendLog throttle (1/hour/kind/scope) 実装済
...
| ~~W20-2~~ | ~~見学当日のクイックキャプチャ (写真+GPS+timestamp)~~ → ✅ 完了確認 (2026-05-05): `visit-section.tsx` の Quick capture bar (line 501-572、`capture="environment"` + `useGeolocation`) + `addVisitNote` (lat/lng) + `addVisitNotePhoto` (写真→VisitNoteMedia) 全配線済 | 中 | ✅ 完了 |
| W20-5 | 見学メモ orphan 画像クリーンアップ routine | 小 | バックグラウンド cron として追加 (W20-2 完了済なので優先度低、画像 leak は問題化してから対応で十分) |

...
## E2E flaky test の隔離フロー (Phase 4/5)

CI の `e2e-test` job が flaky で PR が間欠的に block されたら、**quarantine
タグでまず止血** → 後追いで根治、というパターンに従う。

### 隔離手順

1. Playwright spec で対象 test に tag を追加:

   ```ts
   test("flaky reason for now", { tag: "@quarantine" }, async ({ page }) => {
     // ...
   });
   ```

2. PR を出す (この commit 単独で OK)。CI で:
   - `e2e-test` job が `--grep-invert "@quarantine"` で対象を除外 → green
   - `e2e-quarantine` job が同じ test を `continue-on-error: true` で実行 →
     fail しても PR は merge 可

3. PR 説明文に **「quarantine 解除 issue を併せて open する」** ことを書く。
   issue タイトル: `[E2E quarantine] <spec name> — flaky since <YYYY-MM-DD>`

### 解除手順

flake の根因が分かって直したら:

1. spec から `{ tag: "@quarantine" }` を削除
2. `e2e-test` で 3 回連続 green を確認 (CI の re-run で 3 度回す or local で
   `npx playwright test --repeat-each 3 <spec>`)
3. quarantine 解除 issue を close

### 注意

- quarantine は **「直す前提の止血」** であって「諦め」ではない。issue を
  open しないなら `.skip()` で消すべき (= 当面 / 永久に走らせない宣言)
- quarantine 期間が 30 日を超えたら `.skip()` への降格を検討 (CI 時間の無駄)

...
既に ✅ だが evidence string を ZIP bundle + GDPR-light delete に強化。
...
| 1.3 | データエクスポート (個人情報保護法 27 条 / GDPR Art.20) | ✅ | `src/app/api/user/export/route.ts` で ZIP bundle 配信 (profile/projects/venues/visits/reviews/ratings/coach-messages/notifications.json + photos/manifest.txt + 日本語 README)。 round 15 で JSON 一括 → ZIP 化。 `audit_logs` 行も同 PR で残る |
| 1.4 | データ削除 (同 30 条 / GDPR Art.17) | ✅ | `src/app/api/user/delete/route.ts` で BotID + confirm + email 一致 + 削除実行。 round 15 GDPR-light: partner が project に居る場合は cascade せず detach、 sole member なら cascade。 email anonymise → user.delete の順で実行、 audit row 3 種類 (requested / completed / failed) 永続記録 |
...
❌ 未着手 9 件のうち、 launch ブロッカーになる順 (P2 round 15 で 2.9 が ✅ に昇格 → 残 priority 上位 7 件):
...
5. **4.9 DB backup 検証** — 月次 drill 含めて
6. **6.7 退会後データ保持ポリシー** — 利用規約に明示
7. **5.9 404 ブランドデザイン** — UX polish

...
| ID | 課題 | I×E | 対応 Sprint |
|---|---|---|---|
| F-08 | 決めるタブにフィルタ・設定（ドレス持込料・支払い方法） | M×M | Sprint 3.5（Venue/Estimate スキーマ拡張） |
| F-09 | チェック差分 SEED データ不足 | M×M | Sprint 4 |
| B-17 | Safari 戻るボタンで次の画面 | M×M | Sprint 5 後半 |
| B-18 | 「次へ」挙動不安定 | M×M | Sprint 5 後半 |
| B-11 | タブ切替が遅い | M×M | Sprint 5 後半（Phase 1 計測次第） |
| — | プラン透明化（ドレス 2 着 / 新郎分含有） | M×M | Sprint 3.5（EstimateItem スキーマ拡張） |

### P4 — 低影響 / Later

| ID | 課題 | I×E | 対応 Sprint |
|---|---|---|---|
| B-14 | アプリ内アニメ欠如（Apple 風） | L×L | Sprint 5 後半（tokens 適用の個別深掘り） |
| B-22 | 全コピー素敵化 | H×L | Sprint 5 継続（Phase C/D 残） |
...
- ✅ 業務用語置換 Phase A/B → プロジェクト/保存/削除/エラー等を全画面で softenize
...
- ✅ 口コミ AI 要約 バッチ基盤 → `batchAnalyzeVenueReviews(venueId, {force})` 追加（UI wiring 待ち）

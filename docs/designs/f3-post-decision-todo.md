...
| ケース | 挙動 |
|---|---|
| **決定キャンセル** (`cancelDecision`) | **同一 venue で再決定するケース** と **別 venue に変更して再決定するケース** を区別する:<br>  - (a) **同一 venue 再決定**: todo を保持（`completedAt` もそのまま）。数時間迷って戻るケースに該当。<br>  - (b) **別 venue 再決定**: `DecisionTodo` の `completedAt` を **一括 reset**（`updateMany({where: {projectId}, data: {completedAt: null, completedBy: null}})`）。理由: 「契約書確認（A 式場）」を済ませた状態が「契約書確認（B 式場）」に継承されるのは意味論的に誤り。<br>判定は `cancelDecision → makeDecision` の連続時に**前後の `venueId` 比較**で行う。UI 上は決定がない間 `/preparation` は「まだ決定前です」ステートに戻る。<br>**実装メモ (2026-04-30)**: 比較材料は `Project.last_cancelled_venue_id` カラムに保持。cancel → make の間にサーバープロセスが死ぬケースまで耐える。Phase 2 で `Decision` 履歴テーブル化するときに同テーブルへ移管予定。<br>**将来の再正規化**: `DecisionTodo.decisionId` FK 化は Phase 2。`Decision` が upsert されるたびに新しい row を残す設計（履歴型）に切り替えるときに同時対応。|
